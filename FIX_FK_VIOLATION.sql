-- ============================================
-- FIX: lesson_access_status foreign key violation
...
RETURNS VOID AS $$
DECLARE
  v_module_content_id UUID;  -- The module_content_items.id (for FK)
  v_module_id UUID;
  v_position INTEGER;
  v_lesson_id UUID;
...
BEGIN
  ...
  -- Get the module_content_items.id that references the LESSON (not the quiz)
  ...
  -- Get next content item in the module
  ...
  IF v_next_content_item IS NOT NULL AND v_next_content_item.content_type = 'lesson' THEN
    ...
    -- Use v_module_content_id (from module_content_items) for the FK constraint
...
-- Manually fix all existing 'passed' submissions
DO $$
DECLARE
  r RECORD;
BEGIN
