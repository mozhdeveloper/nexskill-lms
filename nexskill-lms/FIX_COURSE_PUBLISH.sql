-- =====================================================
-- IMMEDIATE FIX: Auto-publish modules/lessons when course is published
-- =====================================================
...
SELECT 
    ...
    (SELECT COUNT(*) FROM module_content_items mci JOIN modules m ON mci.module_id = m.id WHERE m.course_id = c.id AND mci.is_published = true) AS content_published,
    ...
    (SELECT COUNT(*) FROM quizzes q JOIN module_content_items mci ON q.id = mci.content_id JOIN modules m ON mci.module_id = m.id WHERE m.course_id = c.id AND q.is_published = true) AS quizzes_published
FROM courses c
