...
interface CourseEnrollmentCardProps {
  courseId: string;
  courseTitle: string; // ← NEW: required for PaymentModal
  price: number;
  ...
  checking: boolean;
  enrolling: boolean;
  userId: string; // ← NEW: required for transaction record
  onEnroll: () => void;
  onUnenroll: () => void;
...
const CourseEnrollmentCard: React.FC<CourseEnrollmentCardProps> = ({
  ...
  /**
   * Gate: if the course costs money, open the payment modal.
   * If the course is free (price === 0), enroll directly — same as before.
   */
    ...
    <>
      <div className="bg-white dark:bg-dark-background-card rounded-3xl shadow-card p-6 sticky top-6">
        ...
        {checking ? (
          ...
          <div className="space-y-3 mb-6">
            {/* ← Uses handleEnrollClick instead of onEnroll directly */}
            <button
            ...
            >
              {enrolling ? "Enrolling..." : price > 0 ? `Enroll now · ${formatPrice(price)}` : "Enroll for free"}
            </button>
            <button
      ...
      {/* ── Payment Modal (mounted outside card for proper z-index / scroll) ── */}
      <PaymentModal
        ...
        onPaymentSuccess={() => {
          ...
          onEnroll(); // Calls the actual enrollment after simulated payment
        }}
      />
    </>
  );
