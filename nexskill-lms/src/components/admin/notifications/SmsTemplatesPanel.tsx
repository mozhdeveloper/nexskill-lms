...
interface SmsTemplatesPanelProps {
  ...
  onChange: (updatedTemplates: SmsTemplate[]) => void;
  onOpenEditor: (template?: SmsTemplate) => void;
}

const SmsTemplatesPanel: React.FC<SmsTemplatesPanelProps> = ({
  templates,
  onChange,
  onOpenEditor,
}) => {
  ...
  const getCategoryConfig = (category: SmsTemplate['category']) => {
    switch (category) {
      case 'otp':
        return { label: 'OTP', bg: 'bg-[#DBEAFE]', text: 'text-[#1E40AF]' };
      case 'reminder':
        return { label: 'Reminder', bg: 'bg-[#FEF3C7]', text: 'text-[#92400E]' };
      case 'marketing':
  ...
  const getStatusConfig = (status: SmsTemplate['status']) => {
    switch (status) {
      case 'active':
  ...
  const handleToggleStatus = (templateId: string) => {
    const updated = templates.map((t) => {
      if (t.id === templateId) {
        const newStatus: SmsTemplate['status'] = t.status === 'active' ? 'archived' : 'active';
        return { ...t, status: newStatus };
      }
      return t;
    });
    onChange(updated);
    console.log('Toggled SMS template status:', templateId);
  };
    ...
    <div className="bg-white rounded-2xl border border-[#EDF0FB] shadow-md">
      ...
      <div className="p-6 border-b border-[#EDF0FB]">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xl font-bold text-[#111827]">SMS Templates</h2>
            <p className="text-sm text-[#9CA3B5] mt-1">Manage SMS notification templates</p>
          </div>
          <button
      ...
      <div className="p-6">
        {filteredTemplates.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-sm font-semibold text-[#111827] mb-1">No SMS templates found</p>
          ...
          <table className="w-full">
            ...
            <tbody>
              {filteredTemplates.map((template) => {
                ...
                const segments = calculateSegments(template.characterCount);
                ...
                return (
                  <tr key={template.id} className="border-b border-[#EDF0FB] hover:bg-[#F5F7FF]">
                    ...
                    <td className="py-4 px-4">
                      <div className="text-sm text-[#5F6473] font-mono max-w-xs truncate">
                        {template.body}
                      </div>
                    </td>
                    <td className="py-4 px-4">
                      <div className="text-xs text-[#5F6473]">
                        {template.characterCount} chars · {segments} segment
                        {segments !== 1 ? 's' : ''}
                      </div>
                    </td>
                    <td className="py-4 px-4">
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-[#5F6473] font-mono mb-2 line-clamp-2">
                      {template.body}
                    </div>
                  </div>
                  <span
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs text-[#5F6473]">
                    {template.characterCount} chars · {segments} segment{segments !== 1 ? 's' : ''}
                  </span>
