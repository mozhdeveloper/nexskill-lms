...
const CurriculumEditor: React.FC<CurriculumEditorProps> = ({
    ...
    // Helper to get embed URL from various video platforms
    const getEmbedUrl = (url: string): string | null => {
        const trimmedUrl = url.trim();
    ...
    const handleSaveVideoUrl = async (moduleId: string, lessonId: string) => {
        ...
        // Then trigger a save to the database
        if (onSaveVideoBlock) {
            try {
                await onSaveVideoBlock(moduleId, lessonId, contentOptions.videoUrl);
            } catch (err) {
                console.error("Failed to save video block:", err);
                alert("Video URL saved locally but failed to save to database");
            }
        }
                                                                        ...
                                                                        <div className="mt-3 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-gray-100 dark:bg-slate-800">
                                                                            ...
                                                                            <div className="aspect-video bg-black">
                                                                                {(() => {
                                                                                    ...
                                                                                    // Otherwise use iframe for YouTube/Vimeo
                                                                                    return (
                                                                                        <iframe
                                                                                            src={embedUrl}
                                                                                            title="Video preview"
